...
describe("isApiError", () => {
  it("returns true for valid ApiError objects", () => {
    ...
    expect(isApiError({ message: "Error message", request_id: "123" })).toBe(
      true,
    );
  });
...
describe("getErrorMessage", () => {
  it("extracts message from ApiError", () => {
    expect(getErrorMessage({ message: "API error occurred" })).toBe(
      "API error occurred",
    );
    expect(getErrorMessage({ message: "Error", request_id: "123" })).toBe(
      "Error",
    );
  });
