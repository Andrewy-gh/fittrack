...
import { authStatePath, hasAuthState } from "../helpers/local-e2e-auth";
...
test.describe("Authenticated - Workouts CRUD", () => {
  ...
  test("supports create, read, update, delete", async ({ page }, testInfo) => {
    ...
    await page
      .getByRole("combobox", { name: /workout focus options/i })
      .click();
    await page
      .getByRole("combobox", { name: /workout focus search/i })
      .fill(focus);
    await page.getByRole("option", { name: `Create "${focus}"` }).click();
