...
test.describe('Demo Mode - Workout Create', () => {
  ...
  test('should reorder exercises and persist the saved order', async ({
    ...
    const exerciseCards = page.getByTestId('new-workout-exercise-card');
    await expect(exerciseCards).toHaveCount(2);
    await expect(exerciseCards.nth(0)).toContainText('Bench Press');
    await expect(exerciseCards.nth(1)).toContainText('Barbell Squat');
    await expect(
    ...
    const dragHandles = page.getByTestId('new-workout-exercise-card-drag-handle');
    ...
    await expect(exerciseCards.nth(0)).toContainText('Barbell Squat');
    await expect(exerciseCards.nth(1)).toContainText('Bench Press');
    await expect(
      page.getByTestId('new-workout-exercise-card-drag-handle')
    ).toHaveCount(0);
    ...
    const orderedNames = await page.evaluate(() => {
      ...
      const sets = JSON.parse(localStorage.getItem('fittrack-demo-sets') ?? '[]');
      const exercises = JSON.parse(
        localStorage.getItem('fittrack-demo-exercises') ?? '[]'
      );
      const latestWorkoutId = Math.max(
        ...workouts.map((workout: { id: number }) => workout.id)
      );

      return Array.from(
        new Map<number, string>(
          sets
            .filter((set: { workout_id: number }) => set.workout_id === latestWorkoutId)
            .sort(
              (
                a: { exercise_order: number; set_order: number },
                b: { exercise_order: number; set_order: number }
              ) => a.exercise_order - b.exercise_order || a.set_order - b.set_order
            )
            .map((set: { exercise_order: number; exercise_id: number }) => [
              set.exercise_order,
              exercises.find(
    ...
    expect(orderedNames).toEqual(['Barbell Squat', 'Bench Press']);
  });
