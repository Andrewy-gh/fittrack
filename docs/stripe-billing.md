...
Webhook processing is idempotent. Processed Stripe event IDs are stored in `stripe_webhook_events`, so Stripe retries should not duplicate access changes. The restricted runtime role reaches that log only through its narrow database functions, not through direct table access; `PUBLIC` and Supabase Data API roles cannot execute those functions.
