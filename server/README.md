...
For narrow-scope scenarios, the sweep uses the same Gemini model as an LLM judge for semantic checks that phrase lists made brittle: refusing meal-plan requests, redirecting to a supported workout focus, narrowing to one workout/session, and asking the user to choose the first workout/session when needed. Judge calls run at temperature 0 and must return strict JSON. If the judge cannot produce a valid verdict after one retry, scoring falls back to the legacy term lists and the score reason includes `(term-list fallback)`. Full reports and compact JSONL entries include `narrow_scope_judge` when the judge produced a verdict.
