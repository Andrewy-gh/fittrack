...
func newInternalMetricsServer(cfg *config.Config, handler http.Handler) *http.Server {
	return &http.Server{
...
func main() {
	...
	metricsSrv := newInternalMetricsServer(cfg, api.metricsHandler())
	...
	go func() {
		logger.Info("starting internal metrics server", "addr", metricsSrv.Addr)
