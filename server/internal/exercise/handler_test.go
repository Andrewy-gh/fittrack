...
func (m *MockExerciseRepository) GetExerciseMetricsHistory(ctx context.Context, exerciseID int32, userID string) ([]ExerciseMetricsHistoryPoint, MetricsHistoryBucket, error) {
...
func TestExerciseHandler_GetOrCreateExercise(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			requestBody:   map[string]string{"name": "Bench Press", "unexpected": "ignored before strict decoding"},
			setupMock:     func(m *MockExerciseRepository) {},
			ctx:           context.WithValue(context.Background(), user.UserIDKey, userID),
			expectedCode:  http.StatusBadRequest,
			expectJSON:    true,
...
func TestExerciseHandler_UpdateExerciseName(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:          "unknown request field",
			exerciseID:    "1",
			requestBody:   map[string]string{"name": "Updated Exercise", "unexpected": "ignored before strict decoding"},
			setupMock:     func(m *MockExerciseRepository, id int32) {},
			ctx:           context.WithValue(context.Background(), user.UserIDKey, userID),
			expectedCode:  http.StatusBadRequest,
			expectJSON:    true,
