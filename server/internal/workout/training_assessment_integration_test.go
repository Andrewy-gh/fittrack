...
func TestAssessmentIntegrationOwnershipAndPeriod(t *testing.T) {
	...
	result, err := NewAssessmentService(db.New(pool), func() time.Time { return now }).Assess(user.WithContext(ownerCtx, owner), exercise, TrainingEvidenceRequest{StartDate: "2026-09-14", Timezone: "America/New_York"})
	...
	_, err = NewAssessmentService(db.New(pool), func() time.Time { return now }).Assess(user.WithContext(ownerCtx, owner), foreignExercise, TrainingEvidenceRequest{StartDate: "2026-09-14", Timezone: "UTC"})
