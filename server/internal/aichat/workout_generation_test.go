...
func TestBuildChatSystemPromptIncludesWorkoutGuardrails(t *testing.T) {
	...
	requiredSnippets := []string{
		...
		"Treat the user's stated equipment, training location, or space constraints as the available context for the draft",
		...
		"Do not ask optional questions about fitness level, preferred exercises, or other movements to avoid",
		"or the stated constraints cannot support a safe, reasonable draft",
		...
		`"mild front-of-knee discomfort, no sharp pain, 35 minutes, dumbbells and machines," call the ` + workoutDraftToolName,
		"Do not ask for fitness level first",
		"recently strained a shoulder and overhead pressing bothers it",
		"call the " + workoutDraftToolName + " tool with shoulder-friendly dumbbell-only choices that do not assume a bench",
		"Do not ask whether any other movements bother them unless the pain description is unclear or severe",
		`"45-minute back workout, cables only, no injuries," call the workout draft tool`,
		"choose cable-only movements instead of asking what other equipment they have",
...
func TestWorkoutDraftToolDescriptionMatchesMVPReadiness(t *testing.T) {
	requiredSnippets := []string{
		...
		"Equipment is optional for mobility, rehab, prehab, stretching, or warm-up requests",
		"injury status",
		"After one follow-up answer",
		"usable injury details are present",
		"optional details like fitness level, preferred exercises, or other movements to avoid should not block tool use",
		"Treat stated equipment, location, and space constraints as sufficient and closed by default",
		"use only that context unless the user explicitly mentions more",
		"Do not ask what other equipment they have unless the requested workout is unsafe, contradictory, or not reasonably buildable",
		"Do not assume unmentioned accessories or equipment such as a bench, rack, cable, or machine",
