...
func TestRLSPerformanceImpact(t *testing.T) {
	...
	t.Run("Benchmark_Connection_Pool_Pressure", func(t *testing.T) {
		...
		for i := 0; i < numConcurrent; i++ {
			workerUserID := fmt.Sprintf("pool-pressure-user-%d", i)
			setupSecurityTestUser(t, pool, workerUserID)
			setupSecurityTestWorkout(t, pool, workerUserID, fmt.Sprintf("Worker %d workout", i))
		}
		...
		for i := 0; i < numConcurrent; i++ {
			...
			go func(workerID int) {
				...
				_, err = conn.Exec(ctx, "SELECT set_config('app.current_user_id', $1, false)", workerUserID)
				if !assert.NoError(t, err) {
					return
				}
				for j := 0; j < requestsPerGoroutine; j++ {
					var count int
					err := conn.QueryRow(ctx, "SELECT COUNT(*) FROM workout").Scan(&count)
...
func TestRLSPolicyBypassPrevention(t *testing.T) {
	...
	t.Run("Direct_Query_Bypass_Prevention", func(t *testing.T) {
		...
		require.NoError(t, err)
		defer conn.Release()
		_, err = conn.Exec(ctx, "SELECT set_config('app.current_user_id', $1, false)", userB)
		require.NoError(t, err)
		...
		bypassAttempts := []struct {
		...
		}{
			{
				...
				query: "SELECT id FROM workout WHERE id = $1",
				args:  []interface{}{workoutA},
			},
			{
				name:  "OR Injection Attempt",
				query: "SELECT id FROM workout WHERE user_id = $1 OR 1=1",
				args:  []interface{}{userB},
			},
			{
				name:  "UNION Injection Attempt",
				query: "SELECT id FROM workout WHERE user_id = $1 UNION SELECT id FROM workout WHERE id = $2",
				args:  []interface{}{userB, workoutA},
			},
			{
				name:  "Function Bypass Attempt",
				query: "SELECT id FROM workout WHERE current_user_id() = $1 OR id = $2",
				args:  []interface{}{userB, workoutA},
			},
		}
		...
		for _, attempt := range bypassAttempts {
			t.Run(attempt.name, func(t *testing.T) {
				rows, err := conn.Query(ctx, attempt.query, attempt.args...)
	...
	t.Run("Runtime_Role_Cannot_Bypass_RLS", func(t *testing.T) {
		ctx := context.Background()
		var superuser, bypassRLS, ownsUnforcedRLSTable bool
			...
			SELECT
				...
				EXISTS (
					...
					WHERE c.relowner = r.oid
					  ...
					  AND c.relname IN ('users', 'workout')
				)
			FROM pg_roles r
			WHERE r.rolname = current_user
		`).Scan(&superuser, &bypassRLS, &ownsUnforcedRLSTable)
		require.NoError(t, err)
		assert.False(t, superuser, "runtime role must not be a superuser")
		assert.False(t, bypassRLS, "runtime role must not have BYPASSRLS")
		assert.False(t, ownsUnforcedRLSTable, "runtime role must not own an RLS table unless FORCE RLS is enabled")
	})

	t.Run("Missing_Context_Denies_Access", func(t *testing.T) {
		ctx := context.Background()
		conn, err := pool.Acquire(ctx)
		require.NoError(t, err)
		defer conn.Release()
		...
		err = conn.QueryRow(ctx, "SELECT COUNT(*) FROM workout").Scan(&count)
		require.NoError(t, err)
		assert.Equal(t, 0, count, "empty user context should see no workouts")
	})
}
...
func TestRLSSessionVariableEdgeCases(t *testing.T) {
	...
	t.Run("Missing_Session_Variable", func(t *testing.T) {
		...
		var count int
		err = conn.QueryRow(ctx, "SELECT COUNT(*) FROM workout WHERE id = $1", workoutID).Scan(&count)
		require.NoError(t, err)
	...
	t.Run("Null_Session_Variable", func(t *testing.T) {
		_, err := conn.Exec(ctx, "SELECT set_config('app.current_user_id', NULL, false)")
		require.NoError(t, err)
	...
	t.Run("Invalid_User_ID_Format", func(t *testing.T) {
		...
		for _, invalidID := range invalidUserIDs {
			t.Run(fmt.Sprintf("InvalidID_%s", invalidID[:min(10, len(invalidID))]), func(t *testing.T) {
				...
				_, err := conn.Exec(ctx, "SELECT set_config('app.current_user_id', $1, false)", invalidID)
	...
	t.Run("Switching_User_Context_On_One_Connection", func(t *testing.T) {
	...
	t.Run("Current_User_ID_Function_Behavior", func(t *testing.T) {
		for _, expected := range []string{userID, ""} {
			_, err := conn.Exec(ctx, "SELECT set_config('app.current_user_id', $1, false)", expected)
			require.NoError(t, err)

			var actual string
			err = conn.QueryRow(ctx, "SELECT current_user_id()").Scan(&actual)
			require.NoError(t, err)
...
func setupSecurityRLS(t *testing.T, pool *pgxpool.Pool) {
	...
	// Local tests commonly run as the table owner. FORCE makes that owner exercise
	// the same policies as the non-owner production runtime role.
	_, err = pool.Exec(ctx, "ALTER TABLE users FORCE ROW LEVEL SECURITY; ALTER TABLE workout FORCE ROW LEVEL SECURITY")
	require.NoError(t, err)
